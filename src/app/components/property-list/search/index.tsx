...
export default function AdvanceSearch({ category }: { category?: string }) {
    ...
    const breadcrumbLinks = [
        { href: "/", text: "Home" },
        { href: "/properties/properties-list", text: "Loan Products" },
    ];
    ...
    return (
        <>
            <HeroSub
                title={(filters?.category) ? filters?.category: "Loan Products"}
                description="Discover our comprehensive range of financial solutions designed to empower your business growth and success"
                ...
                <div className='lg:max-w-screen-xl max-w-screen-md mx-auto'>
                    ...
                    <div onClick={toggleOffCanvas} className={`fixed inset-0 top-0 w-full h-full bg-gray-900 bg-opacity-50 z-50 transition-transform transform ${isOffCanvasOpen ? 'translate-x-0' : 'translate-x-full'} lg:hidden`}>
                        <div className='absolute top-0 right-0 w-3/4 max-w-xs bg-white dark:bg-semidark shadow-lg h-full'>
                            <div className='py-14 px-8'>
                                ...
                                <p className='mb-6 text-2xl font-semibold text-midnight_text dark:text-white'>Loan Product Search</p>
                                <div className='flex flex-col gap-6'>
                                    ...
                                    {/* Search button */}
                                    <div>
                                        <button className='bg-primary hover:bg-blue-700 text-white w-full py-3 px-6 text-base rounded-lg transition duration-300'>
                                            Find Loan Product
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className='lg:grid lg:grid-cols-12 gap-4'>
                        <div className='hidden lg:block lg:col-span-4'>
                            <div className='py-14 px-8 glass-light dark:glass-dark shadow-property rounded-lg border border-border/20 dark:border-dark_border/20'>
                                <p className='mb-6 text-2xl font-semibold text-midnight_text dark:text-white'>Loan Product Search</p>
                                <div className='flex flex-col gap-6'>
                                    {/* Map through keywords */}
                                    {searchData?.keywords?.map((option: any, index: any) => (
                        ...
                        <div className='col-span-12 lg:col-span-8'>
                            <div className="flex lg:flex-nowrap flex-wrap lg:gap-0 gap-6 w-full justify-between items-center pb-8">
                                <div className="flex w-full justify-between px-4 flex-1">
                                    <h5 className='text-xl text-midnight_text dark:text-white font-semibold'>{filteredCount} Loan Products Found</h5>
                                ...
                                <div className="flex-1 flex gap-3 px-4">
                                    <select
                                        name="short"
                                        className="custom-select border border-border dark:border-dark_border dark:bg-darkmode text-midnight_text dark:text-white focus:border-primary rounded-lg p-3 pr-9 glass-light dark:glass-dark"
                                        value={sortOrder}
                                        onChange={(e) => setSortOrder(e.target.value)}
                                    >
                                        <option value="none">Sort by Product Name</option>
                            ...
                            {filteredProperties.length > 0 ?
                                ...
                                <div className='flex flex-col gap-5 items-center justify-center pt-20'>
                                    ...
                                    <p className='text-gray dark:text-gray-300 text-lg'>No loan products found matching your criteria</p>
                                </div>
                            }
                        </div>
                    </div>
