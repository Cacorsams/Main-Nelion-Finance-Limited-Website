...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link href={item.href} className={`text-base flex py-3 font-normal text-midnight_text hover:text-primary dark:text-white dark:hover:text-primary ${path === item.href ? '!text-primary' : ' text-black dark:text-white '} ${path.includes(item.href) && item.href !== '/' ? "!text-primary " : null}`}>
        {t(item.label as any)}
        {item.submenu && (
